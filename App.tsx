...
const App: React.FC = () => {
  ...
  // Lógica para ocultar controles após inatividade
  ...
  const handleGenerateOfferArt = async () => {
    ...
    try {
      ...
      if (currentOffer.category === Category.FRUTAS) {
        ...
        visualFocus = "crystal clear ice-cold beverage, condensation droplets on the bottle and glass, refreshing mist, extreme cold effect, ice cubes around, vibrant commercial lighting";
      } else {
      ...
      const prompt = `Hyper-realistic professional digital signage advertisement for ${currentOffer.name}. ${visualFocus}. Mouth-watering, appetizing, beverage commercial quality. Cinematic lighting, elegant background with subtle sparks or refreshing mist, high resolution 8k, extremely detailed textures. Luxury market style.`;
    ...
    >
            ...
            {[1, 2, 3].map((i) => (
              <React.Fragment key={i}>
                ...
                <span className="text-black font-black text-2xl uppercase tracking-widest">
                  BEBIDAS GELADAS • CARNES FRESCAS TODOS OS DIAS • ACEITAMOS TODOS OS CARTÕES • FRUTAS FRESCAS • CARVÃO •
                </span>
              </React.Fragment>
            ))}
          </div>
      ...
      <div className={`fixed bottom-16 right-8 z-[200] flex flex-col gap-5 transition-all duration-500 ${showControls ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-20 pointer-events-none'}`}>
        <button 
          ...
          title="Girar Tela 90°"
          className="p-6 bg-white/10 hover:bg-white/20 backdrop-blur-xl rounded-full text-white/40 hover:text-white transition-all border border-white/10 shadow-2xl flex items-center justify-center group"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="group-active:rotate-90 transition-transform"><path d="M21 2v6h-6"/><path d="M21 13a9 9 0 1 1-3-7.7L21 8"/></svg>
        </button>
        <button 
          onClick={toggleFullscreen} 
          title="Modo TV (Tela Cheia)"
          className={`p-6 rounded-full border border-white/10 backdrop-blur-xl transition-all shadow-2xl ${isTvMode ? 'bg-red-600 text-white shadow-[0_0_30px_rgba(220,38,38,0.5)]' : 'bg-white/10 text-white/40 hover:text-white hover:bg-white/20'}`}
        >
        ...
        <button 
          ...
          title="Configuração"
          className="p-6 bg-white/10 hover:bg-white/20 backdrop-blur-xl rounded-full text-white/40 hover:text-white transition-all border border-white/10 shadow-2xl flex items-center justify-center"
        >
